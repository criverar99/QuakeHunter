...
define(['./worldwindlib'],
    function(WorldWind) {
        ...
        function EQPlacemark(coordinates, coloring, magnitude, time, query) {
            ...
            var green = ('rgb(77,172,38)'),
                yellow = ('rgb(184,225,134)'),
                orange = ('rgb(241,182,218)'),
                red = ('rgb(208,28,139)'),
            ...
            function getColor() {
                ...
                if (coloring == 'magnitude') {
                    var toMag = query.maxMagnitude;
                    var fromMag = query.minMagnitude;
                    var deltaMag = (toMag - fromMag)/4.0;
                    if (fromMag <= magnitude && magnitude <= fromMag + deltaMag) {
                        color.code = green;
                        color.name = "green";                      
                    } else if (fromMag + deltaMag < magnitude && magnitude <= fromMag + 2*deltaMag) {
                        color.code = yellow;
                        color.name = "yellow";
                    } else if (fromMag + 2*deltaMag < magnitude && magnitude<= fromMag + 3*deltaMag) {
                        color.code = orange;
                        color.name = "orange";
                    } else if (fromMag + 3*deltaMag < magnitude && magnitude <= toMag) {
                        color.code = red;
                        color.name = "red";
                    } else {
                        color.code = white;
